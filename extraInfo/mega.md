...
- **You do not name the file.** The real filename is encrypted inside the link, so `-o` is optional and only needed to override it.
...
- **If you hit the quota** (HTTP 509), rdm backs off and waits rather than failing. Connect a VPN and it notices the new IP and resumes early instead of sitting out the rest of the timer.
